  ...
  useEffect(() => {
    ...
    const mouseDown = (e: MouseEvent) => {
      ...
      if (isClicked.current === 2) {
        ...
        target.style.top = `${e.clientY - coords.current.startY - (target.offsetHeight-4)/2}px`;
        target.style.left = `${e.clientX - coords.current.startX - (target.offsetWidth-4)/2}px`;
